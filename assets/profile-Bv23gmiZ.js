import"./modulepreload-polyfill-N-DOuI4P.js";/* empty css             */import{D as e,H as t,I as n,T as r,_ as i,d as a,i as o,j as s,n as c,o as l,r as u,t as d}from"./firebase-XHjplP5n.js";/* empty css             */var f=document.getElementById(`navRight`),p=document.getElementById(`profilePage`),m=new URLSearchParams(window.location.search).get(`user`);r(c,async e=>{if(!e){window.location.href=`/bloxverse/auth.html`;return}if(await u(e.uid))return;let r=m||e.uid,c=r===e.uid,h=await s(t(l,`users`,r));if(!h.exists()){p.innerHTML=`<p style="text-align:center;color:#8a8c8e;padding:48px;">User not found</p>`;return}let g=h.data(),_=g.username||`Unknown`,v=g.email||``,y=g.bux||0,b=g.friends||[],x=g.followers||[],S=g.following||[],C=g.createdAt?new Date(g.createdAt).toLocaleDateString():`Unknown`,w=g.birthday||``,T=await a(r),E={developer:`<span class="role-badge" title="Developer"><i class="fa-solid fa-code"></i></span>`,admin:`<span class="role-badge" title="Admin"><i class="fa-solid fa-shield-halved"></i></span>`,mod:`<span class="role-badge" title="Moderator"><i class="fa-solid fa-gavel"></i></span>`},D=T.map(e=>E[e]||``).join(``),O=await s(t(l,`presence`,r)),k=`offline`;if(O.exists()){let e=O.data(),t=e.lastSeen?.toMillis?.()??e.lastSeen,n=Date.now()-12e4;k=t&&t<n?`offline`:e.inGame?`in-game`:`online`}let A=_.charAt(0).toUpperCase(),j=o(w)>=13,M=g.trustedFriends||[],N=[];for(let e of M.slice(0,12))try{let n=await s(t(l,`users`,e));if(n.exists()){let t=n.data();N.push({id:e,username:t.username||`Unknown`})}}catch{}let P=[];for(let e of b.slice(0,12))try{let n=await s(t(l,`users`,e));if(n.exists()){let t=n.data();P.push({id:e,username:t.username||`Unknown`})}}catch{}let F=[];for(let e of x.slice(0,12))try{let n=await s(t(l,`users`,e));if(n.exists()){let t=n.data();F.push({id:e,username:t.username||`Unknown`})}}catch{}if(p.innerHTML=`
