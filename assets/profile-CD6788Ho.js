...
onclick="window.location.href='/bloxverse/profile.html?user=${e.id}'">
      ...
      `;let C=o.querySelector(`.avatar-circle`),w=o.querySelector(`.avatar-dropdown`);C.addEventListener(`click`,e=>{e.stopPropagation(),w.classList.toggle(`open`)}),document.addEventListener(`click`,e=>{w.contains(e.target)||w.classList.remove(`open`)})}),window._logout=async()=>{await i(r),window.location.href=`/bloxverse/auth.html`};