let ws = null;
let currentUserId = null;

// Replace this with the Render URL once deployed
// For local testing, use ws://localhost:8080
const SERVER_URL = 'wss://bloxverse.onrender.com';

export function connectMultiplayer(gameId, userId, username, onPlayerUpdate, onPlayerLeave, onPlayerList, onChatMsg) {
  currentUserId = userId;
  const url = `${SERVER_URL}?gameId=${gameId}&userId=${userId}&username=${encodeURIComponent(username)}`;

  ws = new WebSocket(url);

  ws.onopen = () => {
    console.log('Connected to multiplayer server');
  };

  ws.onmessage = async (event) => {
    try {
      let text = event.data;
      if (text instanceof Blob) {
        text = await text.text();
      }
      const data = JSON.parse(text);
      if (data.type === 'leave') {
        onPlayerLeave(data.userId);
      } else if (data.type === 'update') {
        onPlayerUpdate(data.userId, data.x, data.y, data.z, data.ry, data.moving, data.grounded, data.climbState);
      } else if (data.type === 'playerList' && onPlayerList) {
        onPlayerList(data.players);
      } else if (data.type === 'chat' && onChatMsg) {
        onChatMsg(data);
      }
    } catch (e) {
      console.error('Error parsing WS message', e);
    }
  };

  ws.onclose = () => {
    console.log('Disconnected from multiplayer server');
  };
}

export function sendChat(message, username) {
  if (ws && ws.readyState === WebSocket.OPEN) {
    ws.send(JSON.stringify({ type: 'chat', username, message }));
  }
}

export function sendLocalTransform(x, y, z, ry, moving, grounded, climbState) {
  if (ws && ws.readyState === WebSocket.OPEN && currentUserId) {
    const payload = JSON.stringify({
      type: 'update',
      userId: currentUserId,
      x, y, z, ry, moving, grounded, climbState
    });
    ws.send(payload);
  }
}

export function disconnectMultiplayer() {
  if (ws) {
    ws.close();
    ws = null;
  }
}
