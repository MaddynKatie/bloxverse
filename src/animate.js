...
function findRootPart(fbx) {
    let root = null;
    fbx.traverse((o) => {
        if (root || o === fbx || !o.isGroup) return;
        let hasBone = false;
...
fbxLoader.load(playerModelUrl, (fbx) => {
    ...
    const rootPart = findRootPart(fbx);
    if (rootPart) {
        state.bones['HumanoidRootPart'] = rootPart;
        state.restPose['HumanoidRootPart'] = {
            x: rootPart.rotation.x, y: rootPart.rotation.y, z: rootPart.rotation.z,
            px: rootPart.position.x, py: rootPart.position.y, pz: rootPart.position.z,
        };
    }
...
function buildBoneList() {
    ...
    ordered.push(...KNOWN_BONES.filter(n => state.bones[n]));
    const extra = Object.keys(state.bones).filter(n => !KNOWN_BONES.includes(n) && n !== 'HumanoidRootPart');
